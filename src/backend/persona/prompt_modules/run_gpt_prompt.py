...
from persona.prompt_modules.gpt_structure import *
from persona.prompt_modules.print_prompt import *

def get_random_al8phanumeric(i=6, j=6): 
...
def run_gpt_prompt_task_decomp(persona, task, duration, test_input=None, verbose=False): 
    ...
    ftime_sum = sum([fi_duration for _, fi_duration in fin_output])
...
def print_run_prompts(prompt_template, persona, gpt_param, prompt_input, prompt, output):
    print(prompt)
    print(output)

def safe_generate_response(prompt, gpt_param, max_retry, fail_safe, func_validate, func_clean_up):
    return "safe response"

def generate_prompt(prompt_input, prompt_template):
...
def run_gpt_prompt_create_conversation(persona, target_persona, curr_loc, test_input=None, verbose=False): 
    def create_prompt_input(init_persona, target_persona, curr_loc, test_input=None): 
        ...
        init_persona_thought_nodes = init_persona.a_mem.retrieve_relevant_thoughts(target_persona.scratch.act_event[0], target_persona.scratch.act_event[1], target_persona.scratch.act_event[2])
        init_persona_thought = ""
        for i in init_persona_thought_nodes: 
            init_persona_thought += f"-- {i.description}\n"
        
        target_persona_thought_nodes = target_persona.a_mem.retrieve_relevant_thoughts(init_persona.scratch.act_event[0], init_persona.scratch.act_event[1], init_persona.scratch.act_event[2])
...
def run_gpt_prompt_summarize_conversation(persona, conversation, test_input=None, verbose=False): 
    ...
    fail_safe = get_fail_safe()
    output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe, __func_validate, __func_clean_up, True)
    if output != False: 
        return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_agent_chat(maze, persona, target_persona, curr_context, init_summ_idea, target_summ_idea, test_input=None, verbose=False):
    ...
    example_output = "Jane Doe: Hi! John Doe: Hey there! How are you doing?"
    special_instruction = 'The output should be a string that responds to the question. The conversation output starts with <speaker>: <response> and alternates between the two participants.'
    fail_safe = get_fail_safe()
    output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
...
def ChatGPT_safe_generate_response(prompt, example_output, special_instruction, prompt_count, fail_safe, func_validate, func_clean_up, verbose):
    gpt_response = OpenAIGPT(prompt, example_output, special_instruction, prompt_count)

    if func_validate(gpt_response, prompt):
        return func_clean_up(gpt_response, prompt)
    else:
        if verbose:
            print(f"Invalid response. Using failsafe: {fail_safe}")
        return fail_safe
