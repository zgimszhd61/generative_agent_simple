...
def create_folder_if_not_there(curr_path): 
  """
  检查当前路径中是否存在一个文件夹。如果不存在，则创建该文件夹。
  注意，如果curr_path指定了一个文件位置，它将在包含该文件的文件夹上操作。但是即使路径只指定到一个文件夹，该函数也能正常工作。
  参数:
    curr_path: 当前路径，可以是文件或文件夹的路径
  返回值: 
    True: 如果新建了一个文件夹
    False: 如果未新建文件夹
  """
...
def write_list_of_list_to_csv(curr_list_of_list, outfile):
  """
  将列表的列表写入CSV文件。
  与write_list_to_csv_line不同，它一次性将整个CSV文件写入。
  参数:
    curr_list_of_list: 要写入的列表，形式如下:
               [['key1', 'val1-1', 'val1-2'...],
                ['key2', 'val2-1', 'val2-2'...],]
    outfile: 要写入的CSV文件名    
  返回值: 
...
def write_list_to_csv_line(line_list, outfile): 
  """
  将一行写入CSV文件。
  与write_list_of_list_to_csv不同，这会打开一个现有的outfile，然后将一行附加到该文件。
  即使文件不存在，这也能正常工作。
  参数:
    curr_list: 要写入的列表，形式如下:
               ['key1', 'val1-1', 'val1-2'...]
               这不是一个列表的列表。
    outfile: 要写入的CSV文件名   
  返回值: 
...
def read_file_to_list(curr_file, header=False, strip_trail=True): 
  """
  将CSV文件读取为列表的列表。如果header为True，则返回一个元组(标题行, 所有行)
  参数:
